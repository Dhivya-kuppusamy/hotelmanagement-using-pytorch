...
def add_reservation(g_id, meal, r_id, check_in="now"):
    cmd = f"insert into reservations(g_id,check_in,r_id, meal) values('{g_id}',{f'{chr(39) + check_in + chr(39)}' if check_in != 'now' else 'current_timestamp'},'{r_id}','{meal}');"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
def get_last_reservation():
    cmd = f"select id from reservations order by id desc limit 1;"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return cursor.fetchone()[0]
...
def add_billing(r_id, amount, method):
    cmd = f"insert into billings(b_id,amount,method) values('{r_id}','{amount}','{method}');"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
