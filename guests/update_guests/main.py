...
class UpdateGuests(Frame):
    def __init__(self, parent, controller=None, *args, **kwargs):
        ...
        self.id_entry_image = PhotoImage(file=relative_to_assets("entry_1.png"))
        id_entry_image = self.canvas.create_image(
            212.8127899169922, 288.0, image=self.id_entry_image
        )
        id_entry = Entry(
            self,
        ...
        id_entry.place(
            x=71.56398010253906, y=150.0, width=282.49761962890625, height=22.0
        )
    ...
    def handle_update(self):
        data = [
            x
            for x in [
                self.data[label].get() for label in ("id", "name", "address", "phone")
            ]
        ]

        result = db_controller.update_guests(
            name=data[1], address=data[2], phone=data[3], id=data[0]
        )
        if result:
            ...
            messagebox.showerror(
                "Error",
                "Failed to update guest. Please Verify that all IDs are correct.",
            )
